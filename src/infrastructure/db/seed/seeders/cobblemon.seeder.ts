...
export const cobblemonSeeder: Seeder = {
	...
	tables: ['species_lighting', 'species_riding', 'drop_items'],
	...
	async seed(db, logger) {
		...
		{
			const start = Date.now()
			const data = await loadJson<DropItemsJson>('drop_items.json')
			const rows = data.items.map((i) => ({
				id: i.id,
				source: i.source,
			}))
			const count = await batchInsert(db, 'drop_items', rows)
			logger.table('drop_items', count, Date.now() - start)
			total += count
		}
