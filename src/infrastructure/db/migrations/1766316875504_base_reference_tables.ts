...
export async function up(db: Kysely<any>): Promise<void> {
  ...
  await db.schema.createIndex('idx_move_categories_slug').on('move_categories').column('slug').execute();
  await db.schema.createIndex('idx_move_targets_slug').on('move_targets').column('slug').execute();
  await db.schema.createIndex('idx_ability_flag_types_slug').on('ability_flag_types').column('slug').execute();
  await db.schema.createIndex('idx_move_flag_types_slug').on('move_flag_types').column('slug').execute();
  ...
  await db.schema.createIndex('idx_experience_groups_slug').on('experience_groups').column('slug').execute();
  await db.schema.createIndex('idx_form_tag_types_slug').on('form_tag_types').column('slug').execute();
  await db.schema.createIndex('idx_recipe_tag_types_slug').on('recipe_tag_types').column('slug').execute();

  await sql`CREATE INDEX idx_types_name_trgm ON types USING gin (name gin_trgm_ops)`.execute(db);
  await sql`CREATE INDEX idx_move_categories_name_trgm ON move_categories USING gin (name gin_trgm_ops)`.execute(db);
}
