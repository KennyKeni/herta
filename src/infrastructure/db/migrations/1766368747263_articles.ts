...
export async function up(db: Kysely<any>): Promise<void> {
	...
	await db.schema
		...
		.addColumn('article_id', 'integer', (col) => col.notNull().references('articles.id').onDelete('cascade'))
		.addColumn('category_id', 'integer', (col) => col.notNull().references('article_categories.id').onDelete('cascade'))
	...
	await sql`CREATE INDEX idx_articles_title_trgm ON articles USING gin (title gin_trgm_ops)`.execute(db)
}
